...
2. Increase vm.max_map_count of your local machine(linux), to use elasticsearch.
    ...
    ```
    For macOS, please increase enough memory available for docker (Example, set > 4 GB in docker->preferences->Advanced) or decrease memory for es instance(Example, set -Xms512m -Xmx512m in jvm.options)
