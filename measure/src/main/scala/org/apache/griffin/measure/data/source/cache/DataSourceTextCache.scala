...
case class DataSourceTextCache(sqlContext: SQLContext, param: Map[String, Any],
                               dsName: String, index: Int
                              ) extends DataSourceCache(param, dsName, index) with Loggable {
  ...
  def saveDataFrame(df: DataFrame, path: String): Unit = {
    ...
    // save data
    if (!dataRdd.isEmpty) {
      HdfsFileDumpUtil.dump(path, dataRdd, rowSepLiteral)
    }
  }

  def saveDataList(arr: Iterable[String], path: String): Unit = {
    HdfsFileDumpUtil.dump(path, arr, rowSepLiteral)
  }

  def readDataFrame(paths: Seq[String]): Option[DataFrame] = {
//    paths.foreach(println)
    Some(sqlContext.read.json(paths: _*))
  }

  def removeDataPath(path: String): Unit = {
