...
import org.apache.griffin.measure.utils.{HdfsUtil, JsonUtil, TimeUtil}
...
case class ParquetCacheDataConnector(sqlContext: SQLContext, dataCacheParam: DataCacheParam
...
//  }
  ...
  private def encode(data: Map[String, Any], ms: Long): Option[Row] = {
    try {
      val values = fields.map { field =>
        val StructField(name, _, _, _) = field
        name match {
  ...
  private def decode(data: Row, updateTimeStamp: Boolean): Option[Map[String, Any]] = {
    data.getAs[Any](PayloadColumn) match {
      case v: String => {
        try {
          val map = JsonUtil.toAnyMap(v)
          val resMap = if (updateTimeStamp) {
            data.getAs[Any](TimeStampColumn) match {
              case t: Long => map + (TimeStampColumn -> t)
              case _ => map
            }
          } else map
          Some(resMap)
...
//  private def encode(data: Map[String, Any], ms: Long): Option[CacheData] = {
//    try {
//      val json = JsonUtil.toJson(data)
//      Some(CacheData(ms, json))
//    } catch {
//      case _ => None
//    }
//  }
//
//  private def decode(data: Row, updateTimeStamp: Boolean): Option[Map[String, Any]] = {
//    data.getAs[Any]("payload") match {
//      case v: String => {
//        try {
//          val map = JsonUtil.toAnyMap(v)
//          val resMap = if (updateTimeStamp) {
//            data.getAs[Any]("time") match {
//              case t: Long => map + (TimeStampInfo.key -> t)
//              case _ => map
//            }
//          } else map
//          Some(resMap)
    ...
    if (newCacheLocked) {
      try {
        if (!rdd.isEmpty) {
          ...
          // encode data
          val dataRdd: RDD[Row] = rdd.flatMap(encode(_, ms))
//          val dataRdd: RDD[CacheData] = rdd.flatMap(encode(_, ms))

          // generate data frame
          val df = sqlContext.createDataFrame(dataRdd, schema)
//          val df = sqlContext.createDataFrame(dataRdd)
  ...
  def readData(): Try[RDD[Map[String, Any]]] = Try {
    ...
    val alterPartitionRanges = partitionRanges :+ (reviseTimeRange._1, reviseTimeRange._2)
    println(alterPartitionRanges)

    // list partition paths
    val partitionPaths = listPathsBetweenRanges(filePath :: Nil, alterPartitionRanges)

    if (partitionPaths.isEmpty) {
      sqlContext.sparkContext.emptyRDD[Map[String, Any]]
    } else {
      val df = partitionPaths.map { path =>
      ...
      try {
        ...
        cleanTime match {
          case Some(ct) => {
            ...
            val alterBounds = bounds :+ ct

            // list partition paths
            val earlierPaths = listPathsEarlierThanBounds(filePath :: Nil, alterBounds)
  ...
  private def listPathsEarlierThanBounds(paths: List[String], bounds: List[Long]
    ...
    bounds match {
      ...
      case head :: tail => {
        ...
        earlierPaths ::: listPathsEarlierThanBounds(equalPaths, tail)
      }
    }
  }
}
