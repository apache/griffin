...
case class MariaDBDataConnector(@transient sparkSession: SparkSession,
  ...
  val driver: String = dcParam.getConfig.getString(Driver, "org.mariadb.jdbc.Driver")
  ...
  override def data(ms: Long): (Option[DataFrame], TimeRange) = {
    ...
    val dfOpt = try {
      ...
      case e: Throwable =>
        error(s"load mariadb table $fullTableName fails: ${e.getMessage}", e)
        None
    }
